...
以下の例は石狩第2ゾーン(`is1b`)に、almalinuxパブリックアーカイブをベースとしたアーカイブを作成します。
...
`os_type`が`iso`の場合、`iso_id`、または以下のISOイメージ関連の値の指定が必須です。
  ...
  - `iso_checksum`: ISOイメージのチェックサム。`type:value`の形式で指定します(例: `sha256:abc123...`)。チェックサムファイルを指定する場合は`file:URL`の形式で指定します。チェックサムを行わない場合は`none`を指定
...
PackerがISOイメージをダウンロードし、さくらのクラウド上へアップロードします。
  ...
  - `iso_checksum`: ISOイメージのチェックサム。`type:value`の形式で指定します。チェックサムファイルを指定する場合は`file:URL`の形式で指定します

例:
```json
        "os_type": "iso",
        "iso_url": "http://ftp.tsukuba.wide.ad.jp/software/vyos/iso/release/1.1.7/vyos-1.1.7-amd64.iso",
        "iso_checksum": "sha1:c40a889469e0eea43d92c73149f1058e3650863b",
```
